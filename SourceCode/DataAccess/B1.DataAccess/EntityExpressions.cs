...
namespace B1.DataAccess
{
    ...
    internal class ObjectParser
    {
        ...
        {
            return GetInsertSqlAndParams(new Dictionary<PropertyInfo,object>(), getRowId);
        }
                ...
                Dictionary<PropertyInfo, object> propertyDbFunctions, bool getRowId = false)
        {
                    ...
                    | BindingFlags.Instance))
            {
                ...
                PropertyInfo property = _entityType.GetProperty(prop.Name);
                ...
                if (!column.IsAutoGenerated && !column.IsComputed)
                {
                    ...
                    {
                        if(propertyDbFunctions[property] is string)
                            value = (string)propertyDbFunctions[property];
                        else if (propertyDbFunctions[property] is DbFunctionStructure)
                        {
                            value = ((DbFunctionStructure)propertyDbFunctions[property]).FunctionBody;
                        }
                    }
