...
namespace B1.DataAccess
{
    ...
    internal class ObjectParser
    {
        ...
        {
            return GetInsertSql(new Dictionary<string, object>(StringComparer.CurrentCultureIgnoreCase));
        }

        internal string GetInsertSql(Dictionary<string, object> propertyDbFunctions)
        {
                ...
                if (!column.IsAutoGenerated && !column.IsComputed)
                {
                    ...
                    if (!propertyDbFunctions.ContainsKey(prop.Name))
                    {
                        DbPredicateParameter param = new DbPredicateParameter()
                        {
                            ...
                            ParameterName = LinqTableMgr.BuildParamName(prop.Name, _parameters, _daMgr),
                    ...
                    sbValues.AppendFormat("{0}{1}", sbColumns.Length == 0 ? "" : ", ", value);

                }

            }

            sb.AppendFormat("({0}){1}VALUES ({2}){1}", sbColumns, sbValues, Environment.NewLine);
            return sb.ToString();
        }

        internal string GetUpdateSql()
        {
            return GetUpdateSql(new Dictionary<string, object>(StringComparer.CurrentCultureIgnoreCase));
        }

        internal string GetUpdateSql(Dictionary<string, object> propertyDbFunctions)
        {
            StringBuilder sb = new StringBuilder();
