...
declare @sql nvarchar(1024), @name varchar(64), @type varchar(4)
		, @schema varchar(12), @schema_CG varchar(12)@tableName varchar(64)
set @schema = 'B1'
set @schema_CG = 'dbo'
...
select name, type, object_name(parent_object_id) as tableName from sys.objects 
where schema_id = schema_id(@schema_CG)
and type in ('U', 'P', 'FN', 'F')
order by type

open b1
fetch b1 into  @name, @type, @tableName
...
open cg
fetch cg into  @name, @type, @tableName
while @@fetch_status = 0
begin
